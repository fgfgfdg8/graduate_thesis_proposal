...
\title{面向间接提示注入攻击下LLM Agent工具调用链的行为安全控制研究} % 文档元信息标题
% \author{吴丰艺}
% \date{2026年5月}
\date{} % 不在正文自动生成日期
...
\newcommand{\ThesisTitle}{面向间接提示注入攻击下LLM Agent工具调用链的行为安全控制研究} % 开题报告论文题目
\newcommand{\StudentID}{} % 首页学号
\newcommand{\MajorName}{} % 学科专业
\newcommand{\StudentName}{} % 研究生姓名
\newcommand{\AdvisorName}{} % 导师姓名
...
\input{partitions/section1_background}
\input{partitions/section2_related_work}
\input{partitions/section3_content}
% \input{partitions/section4_research_goals}
% \input{partitions/section5_technical_approach}
\input{partitions/section6_feasibility}
\input{partitions/section7_difficulties}
\input{partitions/section8_outcomes}
\todo{引用条目的会议都采用Proceedings of the……格式，期刊采用Journal of……格式，注意大小写规范}
\input{partitions/section9_references}
\input{partitions/section10_sign}
\clearpage
\showtextframefalse
\input{partitions/section11_workplan}
\input{partitions/section12_reviewer_comments}
