...
针对LLM智能体工具调用链安全问题，本课题拟围绕“威胁感知的工具调用链风险校准与分级防御（TARC-Guard）”和“基于行为轨迹图的工具滥用威胁归因与自适应防御（TraceGraph-Guard）”两个研究点展开，从序列层面建立可校准的风险度量体系并触发分级防御动作，从图结构层面实现工具滥用检测、威胁归因与最小干预防御，两者共同构成可插拔的Defense Guard中间层。下面将从基于大语言模型的智能体能力与工具调用评测、基于大语言模型的智能体安全威胁与防御、基于大语言模型的智能体的轨迹可观测性、执行溯源与威胁归因和基于大语言模型的智能体工具调用风险度量与可解释评估四个方向，逐一梳理整个领域内的相关研究现状。图~\ref{fig:research_roadmap}展示了四条研究脉络的演进路径及其与本课题的关联。
...
\begin{enumerate}
  \item \textbf{G1：针对Agent工具调用链的风险度量体系评估指标粒度较粗，难以捕获过程间组合性风险，且缺乏与防御动作直接挂钩的分级机制。}
  现有针对Agent工具调用链安全问题的基准和防御方法以攻击成功率为核心指标，而对工具敏感度、权限扩散度、调用深度、不可逆操作比例、敏感数据暴露、任务必要性偏差等过程性风险指标的度量研究较少。例如，“语义权限提升”问题这类“序列合法但组合有害”的风险对现有以单次调用为粒度的访问控制系统完全不可见，也无法被任务完成率或攻击成功率等结果导向指标所量化。更重要的是，即使能够输出风险分数，现有方法也缺乏将风险等级直接映射为可操作防御动作（放行/记录/脱敏/人工确认/阻断回滚）的分级机制，导致风险感知与防御响应之间存在断层。
...
\begin{enumerate}
  \item \textbf{创新点一：威胁感知的工具调用链风险校准与分级防御策略（TARC-Guard）。}
  针对G1和G2，本课题提出统一日志模式（新增\texttt{src}/\texttt{sink}/\texttt{confirm}字段）和七维风险因子体系，将工具敏感度$S_{\text{tool}}$、调用深度$D_{\text{call}}$、权限扩散度$P_{\text{diff}}$、敏感数据暴露度$E_{\text{leak}}$、不可逆操作比例$I_{\text{irrev}}$、人工确认覆盖率$H_{\text{cov}}$、任务必要性偏差$N_{\text{need}}$纳入可校准加权评分框架$R_\theta(C) = \sigma(b + \theta^\top \Phi(C))$，并通过“威胁先验初始化—弱监督标签生成—约束优化校准—验证集阈值搜索”四步整定策略学习权重。区别于现有以攻击成功率为核心的评估方案\cite{balunovic2024agentdojo,zhang2024injectagent,liu2024formalizing}，TARC-Guard在序列层面对工具调用链的组合性威胁风险进行可校准量化，并将风险等级（L0--L4）直接映射为分级防御动作（Allow/Log/Mask/AskConfirm/Block+Rollback），契合有关文献\cite{chu2026systematic}对跨步骤组合风险度量和过程级可解释评估层面的新阶段研究趋势。

  \item \textbf{创新点二：基于行为轨迹图的工具滥用威胁归因与自适应防御方法（TraceGraph-Guard）。}
  针对G3和G4，本课题将Agent执行轨迹建模为带防御节点的异构行为轨迹图$G_t=(V_t,E_t,X_t,R_t)$，其中$V_t$新增防御节点$V_{\text{guard}}$，$E_t$新增防御边$E_{\text{defend}}$，使防御动作在图结构中显式可见。提出三级升级判定检测机制（TTED）：第一级图规则库（8条核心规则，覆盖A1--A6六类攻击）；第二级类型化路径特征+轻量分类器（GAT作为可选增强）；第三级LLM终审（仅对边界样本激活）。图级综合风险评分融合序列先验：$M(G_t) = \lambda_1 S_{\text{rule}} + \lambda_2 S_{\text{cls}} + \lambda_3 S_{\text{llm}} + \lambda_4 R_\theta(C)$。在识别风险行为的基础上，进一步提出基于反事实消融的可解释归因方法和最小干预防御公式$a_t^* = \arg\min_{a \in \mathcal{A}} \{\lambda_s \cdot ResidualRisk + \lambda_u \cdot UtilityLoss + \lambda_c \cdot Cost\}$，输出包含归因路径、关键节点、防御动作和恢复计划的完整证据链报告$Defense(G_t)$。TARC-Guard与TraceGraph-Guard共同构成可插拔的Defense Guard中间层（Pre-call/In-call/Post-call/Evidence Guard四阶段），填补现有工作\cite{dewitt2026openchallenges,he2025sentinelagent,wang2026aligning,wang2026agenttrace,hong2026trace,li2026traces,liu2026auditing}在跨步骤风险路径识别、可解释威胁归因和最小干预防御方面的空白。
