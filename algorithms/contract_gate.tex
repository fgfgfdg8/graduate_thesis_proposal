...
\Fn{\FMed{$\text{proposal}$, $K_t$, $\text{state}$}}{
  ...
  \ForEach{$r \in R_t$}{
    $\text{Match}(r, a_t) \leftarrow \text{RouteOK} \land \text{EffectOK} \land \text{ArgOK} \land \text{OriginOK} \land \text{UseOK} \land \text{ConfirmOK}$\;
  }
  \uIf{$\exists r: \text{Match}(r, a_t)$}{
    $\text{status} \leftarrow \texttt{AUTHORIZED}$\;
  }
  \uElseIf{唯一缺口为来源取值、精确调用确认或受限子集}{
